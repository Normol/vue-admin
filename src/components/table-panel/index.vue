<template>
  ...
  >
    <el-table-column
      v-if="isPresenceSelectionColumnProps"
      ...
      >
        <template slot-scope="scope">
          <component
            ...
            v-on="column.component.events ? column.component.events(scope.row): {}"
          />
          <template v-else>
    ...
    <template v-if="isPresenceRowOperateList">
      ...
      >
        <template slot-scope="scope">
          <el-button
            v-for="(operate, index) in getOperateList(scope.$index)"
            :key="`operate_${index}`"
            :type="operate.type"
            size="small"
...
export default {
  ...
  methods: {
    ...
    onRowOperateClick(index, operate) {
      if (operate.method) {
        operate.method(this.tableProps.data[index], this)
      }
    }
  }
}
